...
-- 사원의 인덱스
SELECT EMPNO, ROWID FROM t_emp; -- ROWID, ROW INSERT 될때마다 부여됨 SELECT 시 기본적인 참조

SELECT EMPNO, ROWID FROM t_emp ORDER BY EMPNO DESC;

SELECT ENAME, ROWID FROM t_emp; -- ename의 알파벳 순서로 안나옴
...
SELECT DNAME , DCODE, rowid FROM T_DEPT2 ;
CREATE UNIQUE INDEX idx_dept2_name ON t_dept2(dname);

INSERT INTO t_dept2 VALUES (9100,'임시매장', 1006, '서울지사');
INSERT INTO t_dept2 VALUES (9101,'임시매장', 1006, '부산지사');
 -- ORA-00001: unique constraint (SCOTT0316.IDX_DEPT2_NAME) violated
...
SELECT  POSITION, DEPTNO FROM T_PROFESSOR ;
 ...
 * 
 */
CREATE INDEX idx_porf_pay_fbi ON t_professor(pay+100);
