...
--COMMENT ON COLUMN test_category.ca_depth IS '몇번쨰 카테고리인지 ';


-- 샘플 데이터  
-- 																								name, depth, parent, order
...
SELECT * FROM TEST_CATEGORY ;
--depth2이면서 부모 uid 가 1 인 카테고리 읽기 (order 오름차순)
SELECT ca_uid "uid", ca_name "name", ca_depth "DEPTH", ca_parent "parent", ca_order "ORDER" FROM TEST_CATEGORY WHERE CA_DEPTH =2 AND CA_PARENT =1 
ORDER BY CA_ORDER ASC;

-- patent 가 업는 애들만 골라기
SELECT  ca_uid, ca_name, ca_depth , NVL2(ca_parent, CA_PARENT ,0),ca_order   FROM TEST_CATEGORY WHERE CA_PARENT IS null;

SELECT ca_uid, ca_name, ca_depth , NVL2(ca_parent, CA_PARENT ,0),ca_order FROM TEST_CATEGORY ;
