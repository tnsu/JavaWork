...
public class DateTime01Main {
	...
	public static void main(String[] args) throws Exception{
		...
		int year = now.getYear(); //1900년이후 경과 년도
		int month = now.getMonth()+1; // 0 이 1월
		int day = now.getDate(); //날씨
		int weekDay = now.getDay(); // 일요일 0
		...
		SimpleDateFormat mFmt = new SimpleDateFormat("YYYY:MM:dd-HH:mm:ss",Locale.KOREA);
		String mTime = mFmt.format(new Date(baseTime));		
		String mTime2 = mFmt.format(new Date(curTime));		
		...
		//parse(oldstring) 문자열을 Date로 변경해줌
		...
		long durationInMillis = 1000003; // 2300 : 2.3초
