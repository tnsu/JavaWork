	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < urls.length; i++) {
			...
			if(url.getPath().length() > 0 ) {
				String fileName = url.getPath().substring(url.getPath().lastIndexOf('/')+1); // 마지막 / 우측을 출력
				String fileBaseName = fileName.substring(0, fileName.lastIndexOf('.')); // 0 ~ . 전까지
				String fileExt = fileName.substring(fileName.lastIndexOf('.')+1);
				System.out.println("\t파일명 : " + fileName);
				System.out.println("\t파일base 명 : " + fileBaseName);
