...
public class Crawl06Main {
	...
	public static void main(String[] args) throws IOException {
		...
		// 데이터. URL 에 파일명이 없는 형태 // 파일이름이 안에 저장되어있음
		String [] dataUrls = {
...
//		for (int i = 0; i < dataUrls.length; i++) app.download2(dataUrls[i]);
	...
	public void download2(String srcUrl) throws IOException {
		...
		byte[] buf = new byte[contentLength]; // 다운로드 받을 파일 크기만큼의 버퍼준비!
		int byteimg;
		...
		bin.close();
		bout.close();
		System.out.println("다운로드 ㅣ " + srcUrl + "\n\t → " + dstFile);
		System.out.println("\t" + contentLength + " btyes \t" + contentType);
	}// end download2
	...
	public void download3(String srcUrl) throws IOException {
		...
		System.out.println("다운로드 | " + srcUrl + " → " + dstFile);
		System.out.println("\t" + imgData.getWidth() + " x " + imgData.getHeight());  // 이미지 가로세로
	}// end download3
