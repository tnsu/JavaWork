...
public class BoardValidator implements Validator {
	...
	// 이 Vaildator 가 제공하는 Class의 인스턴스(clazz) 를 유효성 검사할 수 있는가?
	@Override
	...
	public void validate(Object target, Errors errors) {
		System.out.println("validate()"); // 언제 생성되는 지 확인을 위해서
		WriteDTO dto = (WriteDTO)target;
		
		// 수행할 parmeter 검증들
		
		int uid = dto.getUid();
		if(uid == 0) { //스프링이 동작된 이후에 검증을 한다 떄문에 0으로
			System.out.println("uid 오류");
			// 에러 등록 rejectValue(field, errorCode); // 반드시 키 벨류쌍
			errors.rejectValue("uid", "invalidUid");
		}
		
		String name = dto.getName();
		if(name == null || name.trim().isEmpty()) {
			System.out.println("name 오류 : 반드시 한글자라고 입력하세욧!");
