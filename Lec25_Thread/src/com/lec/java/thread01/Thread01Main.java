...
public class Thread01Main {
	...
	public static void main(String[] args) { // Main 의 쓰레드는 항상 존재함  
		// 총 4개의 쓰레드를 실행함
		// 작업쓰레드 총 3 개
		...
		Thread th2 = new MyThread("안뇽");
		

		// 4. 생성된 인스턴스에서 start() 메소드를 호출
		th1.start(); // Thread 를 실행시킴
		th2.start(); // Thread 를 실행시킴
		
		//메인을 그대로 진행되고 Thread 도 동시진행이됨 . 메인이 먼터 출력되고 Thread 출력됨
		...
		// 만약 아래와 같이 다시 start() 하면 IllegalThreadStateException 발생 //Exception도 별도로 동작하는 것임
//		th1.start();  
